...
public class BigMan : Enemy
{
    ...
    private float HugAttackAnimationLength;
    private float ComboAttackAnimationLength;
    private float SlamAttackAnimationLength;
    ...
    private void Start()
    {
        ...
        {
            if (clip.name == "BigMan_HugAttack")
            {
                HugAttackAnimationLength = clip.length;
            }
            if (clip.name == "BigMan_Combo")
            {
                ComboAttackAnimationLength = clip.length;
            }
            if (clip.name == "BigMan_BigSlam")
            {
                SlamAttackAnimationLength = clip.length;
            }
        }
    }
    public override void NewUpdate()
    {
        ...
        if (isAttacking && !isDead)
        {
            ...
            if (AgroManager.agressiveEnemyCounter >= 3 && !SubState_isAttacking && !isBoss)
            {
                //Strafe
                if (!isStrafing)
                {
    ...
    private IEnumerator SubState_Attacking()
    {
        ...
        if (AttackValue < 33)
        {
            Animator.SetTrigger("BigSlam");
            yield return new WaitForSeconds(SlamAttackAnimationLength);
        }
        else if (AttackValue > 33 && AttackValue < 66)
        {
            
            Animator.SetTrigger("HugAttack");
            yield return new WaitForSeconds(HugAttackAnimationLength);
        }
        else if (AttackValue > 66)
        {
            Animator.SetTrigger("Combo");
            yield return new WaitForSeconds(ComboAttackAnimationLength);
        }
